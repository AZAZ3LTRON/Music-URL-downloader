...
class Spotify_Downloader:
    ...
    def download_track(self):
        ...
        while True:  # Add outer loop for URL input retry
            print("\n" + "="*55)
            Enhanced_Menu.clear_screen()
            Enhanced_Menu.print_header("Track Download", "Single song download")
        ...
        for attempt in range(1, MAX_RETRIES + 1):
            ...
            if isinstance(result, subprocess.CompletedProcess) and result.returncode == 0:
                ...
                another = Enhanced_Menu.get_input("Download another track? (y/n): ", "yn", default=True)
                if another:
    ...
    def download_album(self):
        ...
        while True:
            Enhanced_Menu.clear_screen()
            Enhanced_Menu.print_header("ALBUM DOWNLOAD", "Download complete albums from Spotify")
            ...
            Enhanced_Menu.print_status(f"Starting album download...", "info")
            start_time = time.time()
            output_template = str(self.__output_directory / "{artists}/{album}/{artist} - {title}.{output-ext}")
            
            for attempt in range(1, MAX_RETRIES + 1):
                Enhanced_Menu.print_section(f"Download Attempt {attempt} of {MAX_RETRIES}")
                ...
                if isinstance(result, subprocess.CompletedProcess) and result.returncode == 0:
                    ...
                    another = Enhanced_Menu.get_input("Download another album? (y/n): ", "yn", default=True)
                    if another:
                        continue
                    else:
                        return True
    ...
    def download_playlist(self):
        ...
        while True:
            ...
            print(f"\n{Fore.CYAN}{'='*60}{Style.RESET_ALL}")
            Enhanced_Menu.print_status(f"Starting playlist download...", "info")
            start_time = time.time()
            ...
            for attempt in range(1, MAX_RETRIES + 1):
                ...
                if isinstance(result, subprocess.CompletedProcess) and result.returncode == 0:
                    ...
                    another = Enhanced_Menu.get_input("Download another playlist? (y/n): ", "yn", default=True)
                    if another:
                        continue
                    else:
                        return True
