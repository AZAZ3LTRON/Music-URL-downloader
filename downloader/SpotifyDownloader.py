...
from tools.RateLimiter import youtube_limiter, spotify_limiter, looks_throttled
...
class SpotifyMusicDownloader:
        ...
        for attempt in range(1, self.max_retries + 1):
            ...
            if attempt > 1:
                if self._last_download_stats.get("rate_limited"):
                    ...
                    # Handed to the limiter rather than slept on here: a local
                    # sleep pauses only this call, while a penalty is honoured
                    # by every acquire(), including the YouTube downloader's.
                    # The acquire() inside run_download serves the wait.
    ...
    def _download_item(self, item_type: str, url_prompt: str, output_template: str,
        ...
        while True:
            ...
            # Spotify's Web API, not YouTube - hence the separate bucket.
            ...
            for index, entry in enumerate(valid, 1):
                ...
                if index < total:
                    if rate_limit_streak:
                        ...
                        youtube_limiter.penalize(penalty)
                        Enhanced_Menu.print_status(
                            f"Rate limited - holding off {penalty:.0f}s before the next link",
                            "warning")
                        # The next acquire() serves the cooldown; sleeping here
                        # as well would double the wait.
                    else:
        ...
        for attempt in range(1, self.max_retries + 1):
            ...
            if attempt > 1:
                if self._last_download_stats.get("rate_limited"):
                    ...
                    youtube_limiter.penalize(penalty)
                    Enhanced_Menu.print_status(
                        f"Rate limit detected - holding off {penalty:.0f}s...", "warning")
                else:
                    Enhanced_Menu.print_status(
                        f"Waiting {self.retry_delay}s before retry...", "info")
                    time.sleep(self.retry_delay)
            try:
